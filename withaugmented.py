...
def train(tdl, vdl, layer, patience=5, logdir='.'):
    ...
    for epoch in tqdm.trange(100):
        ...
        with torch.no_grad():
            ...
            if best_val_loss > loss:
                last_updated = 0
                best_val_loss = loss
                torch.save(model.state_dict(), logdir+'/best_model-%.2f.pt'%best_val_loss)
                torch.save(optim.state_dict(), logdir+'/best_optim-%.2f.pt'%best_val_loss)
                best = '*'
            else:
                last_updated += 1
...
if __name__ == "__main__":
    ...
    vds = lee.DS('./data/gap-coreference/pickles/bert-large-val-%d.pkl'%args.bert_layer,
                 './data/gap-coreference/gap-validation.tsv', 0)
    vdl = DataLoader(vds, 5)

    tokenizer = BertTokenizer.from_pretrained('bert-large-cased', do_lower_case=False)
    ds = dataset.DS_Augmented('./data/gap-coreference/gap-test.tsv', tokenizer)
    tds = ds
    if args.d:
    ...
    train(tdl, vdl, patience=args.patience, logdir=args.logdir, layer=args.bert_layer)